function [D,Gamma,err,gerr] = ResIncKsvd1(params,varargin)
...
data = params.data;       %训练样本
ReIncAtom = params.ReIncAtom;   %训练原子个数
mu_coherence=params.mu_coherence; %相干参数
...
if (isfield(params,'ksvd_iternum'))
  ksvd_iternum = params.ksvd_iternum;    %ksvd迭代次数
else
  ksvd_iternum = 10;
end

if (isfield(params,'IPR_iternum'))
 IPR_iternum=params.IPR_iternum;           %IPR迭代次数
else
  IPR_iternum = 10;
end
...
%  initialize the dictionary and determine dictionary size %

 dictsize = size(params.initdict,2);
D = params.initdict(:,1:dictsize);

%%%%%  add judge conditions  %%%%%%
...
G = [];
 ...
 [sparse,psnr,rmse] = add_condition(data,D,Gamma);
      
  if (isfield(params,'codemode'))
  switch lower(params.codemode)
    case 'sparsity'
      if(rmse<=params.control_rmse||size(data,2) < dictsize)
         D=params.initdict;
         return;  
      end
    case 'error'
      if(sparse<=params.control_sparse||size(data,2) < dictsize)
         D=params.initdict;
         return;
      end
    otherwise
 ...
 DicLocation=NotSarse(Gamma,dictsize,ReIncAtom);
...
%%%%%%%%%%%%%%%%% main loop  KSVD training atoms   %%%%%%%%%%%%%%%%%


for iter = 1:ksvd_iternum
  
    if (dictsize==size(params.initdict,2))
        break;
    end
  G = [];
  if (memusage >= MEM_NORMAL)
  ...
  p = randperm(ReIncAtom)+(dictsize-ReIncAtom);
  ...
  [D,cleared_atoms] = cleardict(D,Gamma,data,muthresh,unused_sigs,replaced_atoms,ReIncAtom);
%   D = addconstrain(D,Gamma,data,dictsize1,iternum1);
...
D=(A^0.5)*Q';
correct_part=D((end-n+1):end,(end-ReIncAtom+1):end);
...
[u,d,v]=svd(C);
 W=v*u';
 D=W*D;
correct_part=D(:,(end-ReIncAtom+1):end);
D=[Dictionary,correct_part];
%  D(find(D>=-1e-04&D<=1e-04))=0;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%            add constrain to new atoms        %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function [sparse, psnr,rmse,nnznum] = add_condition(X,D,Gamma)
...
for i = 1:blocksize:size(X,2)
  ...
  psnr=20*log10(255/sqrt(sum(err2)/numel(X)));
  rmse=sqrt(sum(err2)/numel(X));
  nnznum= nnz(Gamma);
  sparse = nnz(Gamma)/size(X,2);
end
